...
const proctorSchema = new Schema(
	{
		...
		// JAKE--- I believe that ObjectId's auto increment with 12 digits automatically in Mongo, so references the Screening Model should work
		screenNum: [
			{
		...
		// JAKE--- I moved the dateTime and createdAt to the Screening Model. If we treat the Proctor like a User and the Screening like a comment/post
		// it would make sense in my mind to attach it to the Screening Model i.e. the screening session.
		// Then, the User Model would be just for the hospital/clinic/etc.
	},
	{
		toJSON: {
			virtuals: true,
